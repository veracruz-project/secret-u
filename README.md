...
- Big integers
  ...
  secret-u supports powers-of-two integers up to SecretU262144. This provides an
  interesting alternative to dynamically-sized bigint libraries, while avoiding
  possible secrecy-leaking pitfalls caused by value-dependent bigint sizes.
...
- Bitsliced and shuffle-based tables
  ...
  simultaneously, reducing the performance impact.
...
- Due to the limits of the constant-time bytecode, all loops are implicitly
  unrolled. This may or may not be faster, but can have some odd side-effects
  such as significant memory usage.

- secret-u is not thread-safe, at all. It probably could be, but I didn't put
  any effort into it. Of course Rust will compile with an error if thread-safety
  is violated.
...
- Offload bytecode compilation
  ...
  Unfortunately there are a couple of Rust-specific challenges: secret-u
...
- Enable precompiled bytecode with a separate generation run

  A less graceful solution, but one that could work with much less effort than
  proc_macro-dependent solutions, is to provide two flags: --generate-precompiled
  and --use-precompiled which change how `compile!` works.

  Instead of `compile!` producing bytecode at runtime, if --generate-precompiled
  is provided, `compile!` writes the generated bytecode to an bytecode file at
  runtime, with --use-precompiled changing `compile!` to load the precompiled
  bytecode file instead of compiling the bytecode on-demand.
