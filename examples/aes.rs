...
impl Aes {
    ...
    fn mix_columns(&self, state: SecretU8x16) -> SecretU8x16 {
        fn xtime(x: SecretU8x4) -> SecretU8x4 {
            (x.clone() << SecretU8x4::const_splat(1))
                ^ (
                    ((x >> SecretU8x4::const_splat(7)) & SecretU8x4::const_splat(1))
                        * SecretU8x4::const_splat(0x1b)
                )
        }

        let mut state = SecretU32x4::cast(state);
        for i in 0..4 {
            let x = SecretU8x4::cast(state.clone().extract(i));
            let sum = x.clone().horizontal_xor();
            let rot = SecretU8x4::cast(
                SecretU32::cast(x.clone()).rotate_right(SecretU32::const_(8))
            );

            state = state.replace(i, SecretU32::cast(
                x.clone() ^ xtime(x ^ rot) ^ SecretU8x4::splat(sum)
            ));
        }

        SecretU8x16::cast(state)
    }
    ...
    pub fn encrypt_aligned(&mut self, data: &mut [SecretU8x16]) {
        ...
        for i in 0..data.len() {
            // encrypt ctr
            self.state = (self.compiled_cipher.as_ref().unwrap())(&self.iv);
            // increment ctr
            self.iv = (self.compiled_inc.as_ref().unwrap())(&self.iv);

            // xor result
            data[i] = (self.compiled_xor.as_ref().unwrap())(&data[i], &self.state);
        }
    }
