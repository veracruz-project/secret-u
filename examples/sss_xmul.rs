...
fn gf256_mul(a: SecretP8, b: SecretP8) -> SecretP8 {
    let x = SecretP16::from(a) * SecretP16::from(b);
    let q = (x.clone() >> SecretU16::const_(8)) * SecretP16::const_(GF256_P_INV);
    SecretP8::from_cast((q >> SecretU16::const_(8))*SecretP16::const_(GF256_P) + x)
}
...
/// a/b = a*b^-1 = a*b^254
///
fn gf256_div(a: SecretP8, b: SecretP8) -> SecretP8 {
...
fn gf256_interpolate(xs: &[SecretP8], ys: &[SecretP8]) -> SecretP8 {
    assert!(xs.len() == ys.len());

    // lazily compiled functions
    #[lazy_compile]
    fn gf256_interpolate_step(li: SecretP8, x0: SecretP8, x1: SecretP8) -> SecretP8 {
        gf256_mul(li, gf256_div(x1.clone(), x0 + x1))
    }

    #[lazy_compile]
    fn gf256_interpolate_sum(y: SecretP8, li: SecretP8, y0: SecretP8) -> SecretP8 {
        y + gf256_mul(li, y0)
    }

    let mut y = SecretP8::zero();
    for (i, (x0, y0)) in xs.iter().zip(ys).enumerate() {
        let mut li = SecretP8::one();
...
fn shares_reconstruct<S: AsRef<[SecretU8]>>(shares: &[S]) -> Vec<SecretU8> {
    ...
    let xs: Vec<SecretP8> = shares.iter()
        ...
        .collect();
    for i in 1..len {
        let ys: Vec<SecretP8> = shares.iter()
            .map(|v| SecretP8::from_cast(v.as_ref()[i].clone()))
            .collect();
