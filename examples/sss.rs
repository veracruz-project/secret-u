...
//
// where polynomial = 0x11b and generator = 0x03
//
...
fn gf256_mul(a: SecretU8, b: SecretU8) -> SecretU8 {
    (a.clone().eq(SecretU8::zero()) | b.clone().eq(SecretU8::zero())).select(
        SecretU8::zero(),
        {
            let x = SecretU16::from(GF256_LOG(a)) + SecretU16::from(GF256_LOG(b));
            GF256_EXP(SecretU8::from_cast(
                x.clone().ge(SecretU16::const_(255)).select(
                    x.clone() - SecretU16::const_(255),
                    x
                )
            ))
        }
...
fn gf256_div(a: SecretU8, b: SecretU8) -> SecretU8 {
    ...
    gf256_mul(a, GF256_EXP(SecretU8::ones() - GF256_LOG(b)))
}
...
fn gf256_interpolate(xs: &[SecretU8], ys: &[SecretU8]) -> SecretU8 {
    ...
    #[lazy_compile]
    fn gf256_interpolate_step(li: SecretU8, x0: SecretU8, x1: SecretU8) -> SecretU8 {
        gf256_mul(li, gf256_div(x1.clone(), x0 ^ x1))
    }

    #[lazy_compile]
    fn gf256_interpolate_sum(y: SecretU8, li: SecretU8, y0: SecretU8) -> SecretU8 {
        y ^ gf256_mul(li, y0)
    }
