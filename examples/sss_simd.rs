...
fn gf256_mul(a: SecretU8x32, b: SecretU8x32) -> SecretU8x32 {
    (a.clone().eq(SecretU8x32::zero()) | b.clone().eq(SecretU8x32::zero()))
        .select(
            SecretU8x32::zero(),
            {
                let x = SecretU16x32::from(GF256_LOG(a)) + SecretU16x32::from(GF256_LOG(b));
                GF256_EXP(SecretU8x32::from_cast(
                    x.clone().ge(SecretU16x32::const_splat(255)).select(
                        x.clone() - SecretU16x32::const_splat(255),
                        x
                    )
                ))
            }
...
fn gf256_div(a: SecretU8x32, b: SecretU8x32) -> SecretU8x32 {
    ...
    gf256_mul(a, GF256_EXP(SecretU8x32::ones() - GF256_LOG(b)))
}
